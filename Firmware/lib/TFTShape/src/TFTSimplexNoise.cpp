...
const double TFTSimplexNoise::F2 = 0.5 * (sqrt( 3.0 ) - 1.0);
const double TFTSimplexNoise::G2 = (3.0 - sqrt( 3.0 )) / 6.0;
const uint8_t TFTSimplexNoise::p[256] = {
...
const Grad TFTSimplexNoise::grad3[12] = {
...
uint8_t TFTSimplexNoise::perm[512] = {0};
uint8_t TFTSimplexNoise::permMod12[512] = {0};

// Initialize permutaion arrays
void TFTSimplexNoise::init() {
...
// Fast floor
int32_t TFTSimplexNoise::fastFloor( double x ) {
    int32_t xi = static_cast<int32_t>(x);
    return x < xi ? xi - 1 : xi;
}

double TFTSimplexNoise::dot( const Grad& g, double x, double y ) {
    return g.x * x + g.y * y;
}

// 2D TFTSimplexNoise noise
double TFTSimplexNoise::noise( double xin, double yin ) {
    double s = (xin + yin) * F2;
